...
function carregar(){
    fetch('../Views/xbox.json').then(response=>response.json()).then(jogos=>{
        ...
        jogos.map(jogo=>{
            ...
            const caminhoImg = '../Midia/img/img_xbox/' + jogo.nome_imagem
     ...
     .then(jogos=>{
         ...
         jogos.map(jogo=>{
             if(conts == jogo.id){
                 ...
                 img.src = '../Midia/img/img_xbox/' + jogo.nome_imagem
