    ...
    public function updateJogosPlay($id, $nome, $desc, $img, $genero, $desenvolvedor)
    {
        ...
        if ($img) {
            ...
            if ($nome == '' && $desc == '' && $genero == '') {
                ...
                $stmt = $this->instancia->prepare('UPDATE playstation SET  descricao = :descricao WHERE fk_id = :id');
                $stmt->bindValue('descricao', $desc);
                $stmt->bindValue(':id', $id);
                $stmt->execute();
            } else if ($genero != '' && $nome == '' && $desc == "") {
                $stmt = $this->instancia->prepare('UPDATE playstation SET genero = :genero WHERE fk_id = :id');
                $stmt->bindValue(':genero', $genero);
                $stmt->bindValue(':id', $id);
                $stmt->execute();
            } else if ($nome != '' && $desc == '' && $genero == '') {
                $stmt = $this->instancia->prepare('UPDATE playstation SET nome = :nome WHERE fk_id = :id');
                $stmt->bindValue(':nome', $nome);

                $stmt->bindValue(':id', $id);
                $stmt->execute();
            } else if ($nome != '' && $desc != '' && $genero != '') {
                $stmt = $this->instancia->prepare('UPDATE playstation SET nome = :nome, descricao = :descricao, genero = :genero, desenvolvedor = :dev WHERE fk_id = :id');
                $stmt->bindValue(':nome', $nome);
                $stmt->bindValue('descricao', $desc);
                $stmt->bindValue(':genero', $genero);
                $stmt->bindValue(':id', $id);
